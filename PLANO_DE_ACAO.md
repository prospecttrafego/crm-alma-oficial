...
- [ ] Mapear todos os `refetchInterval` no frontend e justificar cada um (ou remover).
- [ ] Mapear todos os eventos WebSocket emitidos no backend e sua estratégia no frontend (invalidate vs cache update).
- [ ] Garantir que o contrato de eventos WS esteja alinhado (tipos emitidos no backend = tipos tratados no frontend).
- [ ] Rodar build e registrar tamanho/quantidade de chunks (antes) para guiar mudanças no code splitting.
- [ ] Revisar “double source of truth” em preferências (backend `user.preferences` vs `localStorage`) e documentar regra oficial.
- [ ] Registrar baseline de chunks grandes:
  - [ ] `vendor-tSQUg4m7.js` (~552kb) e por que ele ainda existe como “vendor genérico”
  - [ ] `vendor-recharts`, `vendor-emoji`, `vendor-zod`, `vendor-ui` (confirmar se os splits estão trazendo benefício real)
...
- [ ] Tornar `refetchInterval` do unread count **condicional** (somente quando WS estiver desconectado), ou remover polling.
- [ ] Padronizar atualização: escolher entre
  - [ ] **WS → update otimista** (unreadCount) + refetch sob demanda
  - [ ] **WS → invalidate** (refetch) sem update otimista duplicado
- [ ] Definir e documentar “fonte de verdade” para `unreadCount`.
...
- [ ] Listar eventos que já fazem **cache update** (ex.: `message:created`) e manter consistência.
- [ ] Evitar invalidar listas grandes quando o payload do evento permite update incremental.
...
- [ ] Decidir padrão oficial:
  - [ ] **Opção A (recomendada)**: `ApiClient` como única forma de request (React Query sempre chama `api.get/post/...`).
  - [ ] Opção B: default `queryFn` delega ao `ApiClient` com contrato explícito de `queryKey` (ex.: `[path, params]`).
- [ ] Remover/aposentar `getQueryFn` baseado em `queryKey.join("/")` (ou restringir fortemente o formato de `queryKey`).
- [ ] Padronizar shape de `queryKey` (strings/params) para evitar objetos acidentais.
...
- [ ] Remover `client/src/hooks/useLanguage.ts` (ou transformar em wrapper fino do `LanguageContext`).
- [ ] Garantir que apenas `LanguageContext` faça: prioridade `user.preferences` → `localStorage` → default.
...
- [ ] Medir bundle/chunks “antes” e registrar no baseline (Milestone 0).
- [ ] Revisar `manualChunks()` para evitar granularidade excessiva.
- [ ] Manter splits “high impact” (ex.: charts/emoji/firebase) e simplificar o restante.
...
- [ ] Para `notification:new`, considerar enviar payload mínimo útil:
  - [ ] `unreadCount` atualizado e/ou
  - [ ] a própria notificação recém-criada (quando aplicável).
- [ ] Garantir que o payload não inclua PII sensível desnecessária.
...
- [ ] Otimizar `markMessagesAsRead` para evitar loop N updates (usar update em lote/SQL).
...
- [ ] Alinhar o contrato do evento `google_calendar:sync_complete` no frontend (tipos + tratamento).
- [ ] Decidir estratégia:
  - [ ] **Preferida**: WS dispara invalidation de `["/api/integrations/google-calendar/status"]` e `["/api/calendar-events"]`
  - [ ] e o polling (`refetchInterval: 30000`) vira fallback **somente quando WS estiver offline**
...
- [ ] Tornar polling do QR modal (`refetchInterval: 3000`) “bounded” (somente enquanto `qr_pending/connecting`) — já é assim, validar limites/timeout total de UX.
...
- [ ] Extrair um `EmailSyncService` (ou função) único que:
  - [ ] chama `syncEmails(...)` e delega cada email para `processIncomingEmail(...)`
  - [ ] atualiza `lastSyncUid/lastSyncAt`
- [ ] Fazer o endpoint sync (imediato) e o job async usarem o mesmo serviço (DRY).
...
- [ ] Definir política de produção:
  - [ ] se `NODE_ENV=production` e Redis não está disponível: **fail fast** (ou bloquear endpoints `?async=true`) para evitar job “sumir”
- [ ] Padronizar endpoints async:
  - [ ] listar quais endpoints suportam `?async=true` e garantir UX consistente (jobId/status/erros)
